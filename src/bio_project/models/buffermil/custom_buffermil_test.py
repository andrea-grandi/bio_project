...
class CustomBuffermil(Baseline):
    def __init__(self, args, state_dict_weights):
        ...
        self.mil.buffer = torch.tensor([], device="cuda" if torch.cuda.is_available() else "cpu")
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, levels: torch.Tensor, childof: torch.Tensor, edge_index2: torch.Tensor = None, edge_index3: torch.Tensor = None):
        ...
        results = {}
        if self.inference:
            if self.mil.buffer is None:
                self.mil.buffer = torch.tensor([], device=x.device)
    ...
    def storebuffer(self, loader):
        if self.mil.buffer.size(0) == 0:
            dummy_feats = torch.randn(10, self.args.input_size).to(self.mil.buffer.device)
            self.mil.store(dummy_feats)
    ...
    def storeBuffer(self, feats, scores, k):
        ...
        # MODIFIED
        if self.mil.buffer.size(0) == 0:
            self.mil.buffer = m_feats
        else:
            self.mil.buffer = torch.cat([self.mil.buffer, m_feats])
