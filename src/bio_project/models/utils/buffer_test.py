...
class BClassifierBuffer(nn.Module):
    ...
    def forward(self, feats_slide, feats_buffer, use_buffer, aggregationtype: None): # N x K, M x K
        ...
        if feats_buffer is None or feats_buffer.size(0) == 0:
            use_buffer = False

        if use_buffer:
            feats_buffer = self.lin(feats_buffer)
            Q = self.q(feats_slide).view(feats_slide.shape[0], -1)
            q_max = self.q(feats_buffer)
            A = torch.mm(Q, q_max.transpose(0, 1))
            
            if aggregationtype == "mean":
                A = A.mean(dim=1)[:, None]
            elif aggregationtype == "max":
                A = A.max(dim=1)[0][:, None]

            A = F.softmax(A / torch.sqrt(torch.tensor(Q.shape[1], dtype=torch.float32, device=device)), 0)
            B = torch.mm(A.transpose(0, 1), self.v(feats_slide))
        else:
            # Fallback: modalità senza buffer
            V = self.v(feats_slide)
            B = V.mean(dim=0, keepdim=True)  # Aggregazione semplice
...
class MILNetBuffer(nn.Module):
    def __init__(self, i_classifier, b_classifier):
        ...
        self.buffer = torch.tensor([], device="cuda" if torch.cuda.is_available() else "cpu")
    ...
    def store(self, feats):
        if self.buffer.size(0) == 0:
            ...
            self.buffer = torch.cat([self.buffer, feats])

    def bufferinference(self,feats,inference,aggregationtype):
        if self.buffer.size(0) == 0:
            return self.b_classifier(feats, None, False, aggregationtype)
        else:
            return self.b_classifier(feats, self.buffer, inference, aggregationtype)
