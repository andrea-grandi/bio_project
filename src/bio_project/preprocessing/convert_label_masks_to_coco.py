        ...
        if 'tissue images' in root:
            for file in files:
                if file.endswith('.png'):
                    ...
                    """ 
                        Why the label masks are in .tif?
                        
                        Because in one image you can have 3000 nuclei and with a 8-bit png format 
                        i can have only 2^8 (256), so we have to use .tif :)
                    """
...
def main():
    ...
    train_img_paths, temp_img_paths, train_mask_paths, temp_mask_paths = train_test_split(image_paths, mask_paths, test_size=0.4, random_state=42)
