...
def extract_features_for_buffermil(masks):
    """
    Estrae le features dalle maschere di segmentazione per BufferMIL
    
    Args:
        masks: maschere di segmentazione da Cellpose
        
    Returns:
        features: torch.Tensor con le features estratte
    """
    
    if masks is None or len(masks) == 0:
        return None
        
    # Estrai le features per ogni cellula nella maschera
    features = []
    mask = masks[0]  # prendi la prima maschera
    
    # Trova tutte le cellule uniche (esclude lo sfondo che ha valore 0)
    cell_ids = np.unique(mask)
    cell_ids = cell_ids[cell_ids != 0]
    
    for cell_id in cell_ids:
        # Crea una maschera binaria per la cellula corrente
        cell_mask = (mask == cell_id)
        
        # Calcola varie features
        area = np.sum(cell_mask)
        centroid = center_of_mass(cell_mask)
        perimeter = cv2.countNonZero(cv2.Canny(cell_mask.astype(np.uint8), 100, 200))
        
        # Combina le features in un vettore
        cell_features = np.array([
            centroid[0], centroid[1],  # posizione
            area,                      # area
            perimeter,                 # perimetro
            area / (perimeter + 1e-6)  # compattezza
        ])
...
if __name__ == "__main__":
    ...
    features = extract_features_for_buffermil(masks)
    print(features)