...
def main():
  ...
  ### ----- PREPROCESSING WSI ----- ###
  preprocess_wsi(config['source_dir'], config['save_dir'], config['patch_size'], config['preset'])

    ### ----- CONVERT H5 TO JPG ----- ###
  convert_h5_to_jpg(config['output_dir'], config['source_dir'], config['slide_ext'], config['csv_file'])

  ### ----- SORT HIERARCHY ----- ###
  sort_hierarchy(config['sourcex20'], config['dest'])
  ...
  ### ----- FEATURE EXTRACTION ----- ###
  feature_extraction(
    config['extractedpatchespath'],
    config['savepath'],
    config['pretrained_weights1'],
    config['pretrained_weights2'],
    config['pretrained_weights3'],
    config['propertiescsv']
  )

  ### ----- PREPARE DATASET ----- ###
  prepare_dataset(config['source_feats'], config['dest_feats'])
